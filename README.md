...
本アプリは、アプレンティスでのオリジナルプロダクト作成時点で基本実装を完了しています。  
そのため、今回の課題提出では大きな機能追加ではなく、READMEの内容整理および補足説明を行いました。  

なお、本件についてはRUNTEQ運営事務局へ事前に相談済みであり、既に作成済みの制作物をベースにREADME、画面遷移図、ER図等を整備したうえで提出を進める形で問題ない旨をご案内いただいています。

### 現在の状況
- オリジナルプロダクトは作成済み
- 現時点では大幅な機能変更はなし
- 本PRではREADMEの更新を実施
- 提出方針についてはRUNTEQ運営事務局へ相談済み
